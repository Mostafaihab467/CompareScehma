...
public partial class DiagramViewModel : ObservableObject
{
    ...
    [ObservableProperty] private DiagramTableNode? _isolatedTable;
    [ObservableProperty] private string _isolateBannerText = string.Empty;
    ...
    /// <summary>Set by the view: (suggestedFileName) → chosen PNG path or null.</summary>
    public Func<string, Task<string?>>? PickPngFileAsync { get; set; }
    /// <summary>View subscribes: renders the canvas to PNG when the command fires.</summary>
    public event Func<Task>? ExportPngRequested;
    ...
    public string SuggestedPngFileName => $"{(string.IsNullOrWhiteSpace(Database) ? "diagram" : Database)}.png";
    ...
    partial void OnIsolatedTableChanged(DiagramTableNode? value)
    {
        OnPropertyChanged(nameof(IsIsolated));
        ApplyIsolationDimming();
    }
    ...
    private async Task LoadSchemaAsync()
    {
        ...
        try
        {
            ...
            IsolatedTable = null;
            IsolateBannerText = string.Empty;
    ...
    /// <summary>
    /// Shows all relations of one table: reveals its direct parents + children,
    /// dims everything else and highlights the connected edges.
    /// Clicking the same table again clears the isolation.
    /// </summary>
    [RelayCommand]
    private void IsolateTable(DiagramTableNode? node)
    {
        if (node is null) return;
        if (IsolatedTable is not null &&
            string.Equals(IsolatedTable.FullName, node.FullName, StringComparison.OrdinalIgnoreCase))
        {
            ClearIsolation();
            return;
        }
        var related = GetDirectlyRelated(node.FullName);
        // Reveal related tables so their edges can be drawn.
        foreach (var t in Tables)
        {
            if (related.Contains(t.FullName) && !t.IsVisible)
                t.IsVisible = true;
        }
        IsolatedTable = node;
        var parents = Relations.Count(r =>
            string.Equals(r.ChildTable, node.FullName, StringComparison.OrdinalIgnoreCase));
        var children = Relations.Count(r =>
            string.Equals(r.ParentTable, node.FullName, StringComparison.OrdinalIgnoreCase));
        var names = related.Where(n =>
                !string.Equals(n, node.FullName, StringComparison.OrdinalIgnoreCase))
            .OrderBy(n => n).ToList();
        IsolateBannerText = $"{node.FullName}: {parents} parent(s), {children} child(ren)" +
            (names.Count > 0 ? $" — {string.Join(", ", names.Take(8))}" + (names.Count > 8 ? $" +{names.Count - 8} more" : "") : " — no FK relations");
        StatusMessage = $"Showing relations for {node.FullName} ({related.Count} table(s)). Click 🔗 again or ✕ to show all.";
    ...
    {
        if (IsolatedTable is null && string.IsNullOrEmpty(IsolateBannerText)) return;
        IsolatedTable = null;
        IsolateBannerText = string.Empty;
        StatusMessage = "Showing all tables.";
        ApplyFilter();
    }

    /// <summary>Clamps and applies a zoom level (used by Ctrl+mouse-wheel).</summary>
    public void ApplyZoom(double newZoom) =>
        ZoomLevel = Math.Max(0.25, Math.Min(2.5, Math.Round(newZoom, 2)));
    ...
    private async Task ExportPngAsync()
    {
        if (!HasSchema)
        {
            ErrorMessage = "Load a schema before exporting PNG.";
            ShowError = true;
            return;
        }
        if (ExportPngRequested is not null)
            await ExportPngRequested.Invoke();
    }
    ...
    private void ApplyFilter()
    {
        ...
        HashSet<string>? isolatedSet = IsolatedTable is not null
            ? GetDirectlyRelated(IsolatedTable.FullName)
            : null;
        foreach (var t in Tables)
        {
            ...
            var dimmed = !match;
            if (isolatedSet is not null && !isolatedSet.Contains(t.FullName))
                dimmed = true;
            t.IsDimmed = dimmed;
    ...
    private void ApplyIsolationDimming()
    {
        if (IsolatedTable is null)
        {
            ApplyFilter();
            return;
        }
        var related = GetDirectlyRelated(IsolatedTable.FullName);
        foreach (var t in Tables)
            t.IsDimmed = !related.Contains(t.FullName);
        UpdateCounts();
        PositionsChanged?.Invoke();
    }

    /// <summary>Self + direct FK parents + direct FK children (1-degree neighbourhood).</summary>
    public HashSet<string> GetDirectlyRelated(string fullName)
    {
        var set = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { fullName };
        foreach (var r in Relations)
        {
            if (string.Equals(r.ChildTable, fullName, StringComparison.OrdinalIgnoreCase))
            {
                set.Add(r.ParentTable);
                set.Add(r.ChildTable);
            }
            else if (string.Equals(r.ParentTable, fullName, StringComparison.OrdinalIgnoreCase))
            {
                set.Add(r.ChildTable);
                set.Add(r.ParentTable);
            }
